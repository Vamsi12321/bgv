...
export default function SelfVerificationPage() {
  const [candidate, setCandidate] = useState({
    ...
    mobile: "",
    org: "",
    type: "Primary", // default verification type
  });
  ...
  const handleSubmit = (e) => {
    ...
    alert(
      `✅ Self-verification initiated for ${candidate.name} (${
        candidate.org
      })\nType: ${candidate.type}\nChecks: ${checks.join(", ")}`
    );

    // Reset form
    setCandidate({ name: "", email: "", mobile: "", org: "", type: "Primary" });
    setSelectedChecks({
      aadhaar: false,
      pan: false,
      address: false,
  ...
  const checksList = [
    ...
    },
    {
      ...
      desc: "Verify employee provident fund account (UAN).",
      icon: "🧾",
    },
    {
      key: "epfo",
      title: "EPFO Verification",
      desc: "Verify EPFO account and contributions.",
      icon: "🏦",
    },
    {
      key: "criminal",
      title: "Criminal Check",
      desc: "Check for criminal records.",
      icon: "🛡️",
    },
  ];
      ...
      <div className="bg-white p-6 rounded-xl shadow-md">
        ...
        >
          ...
          <input
            ...
            value={candidate.org}
            onChange={(e) =>
              setCandidate({ ...candidate, org: e.target.value })
            }
          />
          <select
